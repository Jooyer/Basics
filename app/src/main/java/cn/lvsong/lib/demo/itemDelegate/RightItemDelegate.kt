...
class RightItemDelegate : ItemViewDelegate<String> {
    ...
    override fun convert(holder: ViewHolder, bean: String, position: Int) {
        holder.getView<AppCompatImageView>(R.id.aiv_avatar_right_chat).load(R.mipmap.ic_launcher_round){
            transformations(CircleCropTransformation())
        }

        holder.getView<BubbleImageView>(R.id.biv_avatar_right_chat).load(R.mipmap.ic_test)
